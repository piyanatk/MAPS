...
/* RCS: $Id: uvfits.h,v 1.4 2007/02/23 14:59:53 rwayth Exp rwayth $
 */
...
typedef struct _fq_table {
	//int id; // ID (can just be taken as index unless it has to start at 0)
...
typedef struct _uvdata {
    ...
    int  n_IF;            /* number of IFs, each of which can contain a number of channels */
    int  n_freq;          /* number of frequencies within each IF band */
