...
/* output file to write node output.                                    */
/* No FSEEK command is issued to position the output file pointer       */
/* to the correct write position. The file positioning must be done     */
/* upon return from file_offset() with the command:                     */
...
/* Modified 29 Nov 2010 by L. Benkevitch:                               */
/*    fseeko() commented out;                                           */
/*    FILE *outfp removed from the parameter list.                      */
/************************************************************************/
#include <stdio.h>
#include <stdlib.h>
#include <math.h>
...
int
file_offset (/* numproc, myid, refpoint, obsparam, scan_no, nstat, 
	     ...
	     off_t *write_pos)
{
  ...
  for (n=0;n<numproc;n++)
	...
	/* fseeko() commented out by L. Benkevitch to facilitate *
	 * file_offset() usage in the visgen dry-run routine     *
	 * check_baseline(). Therefore, fseek() must be issued   *
	 * in visgen.c (and not in check_baseline()!) right      *
	 * after the call to file_offset() as                    *
	 *      fseeko (outfp, write_pos, SEEK_SET);             *
	 *                                                       */
	...
	*intg_num  = nintg_proc;
	...
	printf ("n = %d,  position = %lld,  slices = %d,  start = %d, " \
       ...
       * visblock struct    */
      *refpoint += nintg_proc * 
	nbase * (bufsize + 8 + sizeof(struct visblock)-sizeof(struct visfreq));
